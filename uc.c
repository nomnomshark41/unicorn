...
uc_err uc_open(uc_arch arch, uc_mode mode, uc_engine **result)
{
    ...
    uc_engine *uc;
    uc_err err = uca_engine_alloc(arch, mode, &uc);
    if (err != UC_ERR_OK)
        return err;

    /* Set the arch-specific init function pointer. */
    switch (arch) {
    default:
        break;
#ifdef UNICORN_HAS_M68K
...
#ifdef UNICORN_HAS_MIPS
            if (mode & UC_MODE_MIPS32) uc->init_arch = uc_init_mips;
#endif
#ifdef UNICORN_HAS_MIPS64
            if (mode & UC_MODE_MIPS64) uc->init_arch = uc_init_mips64;
#endif
        } else {
#ifdef UNICORN_HAS_MIPSEL
            if (mode & UC_MODE_MIPS32) uc->init_arch = uc_init_mipsel;
#endif
#ifdef UNICORN_HAS_MIPS64EL
            if (mode & UC_MODE_MIPS64) uc->init_arch = uc_init_mips64el;
#endif
        }
        break;
#endif
    ...
    case UC_ARCH_SPARC:
        ...
        uc->init_arch = (mode & UC_MODE_SPARC64) ? uc_init_sparc64
                                                 : uc_init_sparc;
        break;
#endif
#ifdef UNICORN_HAS_PPC
    case UC_ARCH_PPC:
        ...
        uc->init_arch = (mode & UC_MODE_PPC64) ? uc_init_ppc64 : uc_init_ppc;
        break;
#endif
#ifdef UNICORN_HAS_RISCV
    case UC_ARCH_RISCV:
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_read_batch(uc, regs, vals, count);
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_write_batch(uc, regs, vals, count);
    restore_jit_state(uc);
    return err;
}
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_read_batch2(uc, regs, vals, sizes, count);
    restore_jit_state(uc);
    return err;
}
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_write_batch2(uc, regs, vals, sizes, count);
    restore_jit_state(uc);
    return err;
}
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_read(uc, regid, value);
    restore_jit_state(uc);
    return err;
}
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_write(uc, regid, value);
    restore_jit_state(uc);
    return err;
}
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_read2(uc, regid, value, size);
    restore_jit_state(uc);
    return err;
}
...
{
    UC_INIT(uc);
    uc_err err = uca_reg_write2(uc, regid, value, size);
    restore_jit_state(uc);
    return err;
}
...
{
    // Avoid nested uc_emu_start saving wrong jit states.
    if (uc->nested_level == 0) {
        UC_INIT(uc);
    }
    uc_err err = uca_emu_start(uc, begin, until, timeout, count);
    if (uc->nested_level == 0)
        restore_jit_state(uc);
    return err;
}
...
{
    UC_INIT(uc);
    uc_err err = uca_query(uc, type, result);
    restore_jit_state(uc);
    return err;
}
